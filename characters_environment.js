...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Click on play button and be in front of the camera to Start and Play ", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(game_status == "started" && noseX != "" && gameConfig.status==="start") {
    world_start.play();
